...
import { ConfirmRebuildFieldsModal } from './confirmRebuildFields';

describe('ConfirmRebuildFieldsModal', () => {
	beforeEach(() => {
		settings.length = 0;
		modalState.title = '';
		modalState.texts = [];
	...
	function openModal(onConfirm: () => void) {
		const modal = new ConfirmRebuildFieldsModal({} as never, onConfirm);
		modal.onOpen();
		return modal;
	}
	...
	it('sets the title and body copy', () => {
		...
		expect(modalState.title).toBe('Rebuild note fields?');
		expect(modalState.texts).toContain(
			'This deletes everything below the note properties and creates one empty section per field of its Model. This cannot be undone.',
		);
	});

	it('renders Cancel and a warning-styled Rebuild button', () => {
		openModal(vi.fn());

		const [buttons] = settings.map((s) => s.buttonComponents);
		expect(buttons?.map((b) => b.text)).toEqual(['Cancel', 'Rebuild']);
	...
	it('closes and calls onConfirm when Rebuild is clicked', async () => {
		const onConfirm = vi.fn();
		openModal(onConfirm);
