...
## Summary
- 938 nodes · 1575 edges · 104 communities (37 shown, 67 thin omitted)
...
5. `FakeEl` - 18 edges
6. `resolveAnkiConnectUrl()` - 17 edges
7. `ProviderError` - 17 edges
...
Cohesion: 0.08
Nodes (22): AnkiConnectClient, { requestUrl }, mapContentToFields(), stringifySectionValue(), extractSectionValue(), parseSections(), readAnkiFrontmatter(), file (+14 more)
...
Cohesion: 0.10
Nodes (34): generateContentSkeleton(), rebuildContent(), runCreateNote(), sanitizeForFilename(), AppWithSettingTab, getQuickCaptureFilename(), getSelectedText(), getUniqueNotePath() (+26 more)
...
Cohesion: 0.06
Nodes (56): plugin, $schema, .opencode/plugins/graphify.js, AnkiBridgePlugin, { loadSettings, saveSettings, getActiveTextConfig, getActiveImageConfig }, { PluginBase, addCommandSpy }, { registerSidebarView, revealSidebarView }, { runCreateNote } (+48 more)

### Community 85 - "src/types.ts"
Cohesion: 0.06
Nodes (48): bearer(), Entry, Fetcher, FETCHERS, FILTERS, get(), listModels(), ModelList (+40 more)
...
Cohesion: 0.16
Nodes (17): AiButtonAction, AiPreCheckResult, runAiPreCheck(), fillEmptySections(), FillResult, resolveSectionKey(), sectionHeadings(), GenerateOutcome (+9 more)
...
Cohesion: 0.18
Nodes (7): renderImageProviderSection(), auto, { listModels }, { Notice, rendered, secrets }, openai, setup(), clearModelCache()
...
## Knowledge Gaps
- **247 isolated node(s):** `$schema`, `.opencode/plugins/graphify.js`, `id`, `name`, `version` (+242 more)
...
- **Why does `AnkiBridgePlugin` connect `settings.ts` to `SidebarView`, `quickCapture.ts`, `settingsTab.test.ts`, `settingsTab.ts`, `sidebarView.test.ts`, `generateFields.ts`, `noteActions.test.ts`, `textTab.test.ts`?**
  _High betweenness centrality (0.040) - this node is a cross-community bridge._
- **Why does `AnkiBridgeSettings` connect `settings.ts` to `quickCapture.ts`, `settingsTab.test.ts`, `sidebarView.test.ts`, `textProviderSection.test.ts`, `generateFields.ts`, `imageProviderSection.test.ts`, `textTab.test.ts`?**
