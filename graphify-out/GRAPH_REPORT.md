...
## Corpus Check
- 82 files · ~47,687 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 503 nodes · 674 edges · 80 communities (24 shown, 56 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 15 edges (avg confidence: 0.86)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e727070c`
...
3. `AnkiConnectClient` - 16 edges
4. `compilerOptions` - 16 edges
5. `runQuickCapture()` - 15 edges
6. `syncNote()` - 13 edges
7. `AnkiBridgePlugin` - 11 edges
8. `toastError()` - 9 edges
9. `Graphify Full Pipeline` - 9 edges
10. `Generate with AI Button` - 9 edges

## Surprising Connections (you probably didn't know these)
- `registerControlsBlock()` --references--> `plugin`  [EXTRACTED]
  src/note/controlsBlock.ts → .opencode/opencode.json
- `registerSidebarView()` --references--> `plugin`  [EXTRACTED]
  src/ui/sidebarView.ts → .opencode/opencode.json
...
Cohesion: 0.10
Nodes (17): AnkiConnectClient, AnkiConnectResponse, { requestUrl }, extractSectionValue(), parseSections(), readAnkiFrontmatter(), file, writeAnkiFrontmatter() (+9 more)

### Community 6 - "SidebarView"
Cohesion: 0.28
Nodes (3): registerSidebarView(), SidebarView, { ItemView, contentElEmpty, contentElCreateEl }
...
Cohesion: 0.12
Nodes (23): ALWAYS_VISIBLE_BUTTONS, ControlAction, ControlButtonSpec, DELETE_BUTTON, handleDeleteClick(), handleSync(), performDelete(), registerControlsBlock() (+15 more)
...
Cohesion: 0.07
Nodes (34): plugin, $schema, .opencode/plugins/graphify.js, AnkiBridgePlugin, { loadSettings, saveSettings }, { PluginBase, addCommandSpy }, { registerControlsBlock }, { registerSidebarView } (+26 more)
...
## Knowledge Gaps
- **175 isolated node(s):** `$schema`, `.opencode/plugins/graphify.js`, `id`, `name`, `version` (+170 more)
...
- **Why does `AnkiConnectClient` connect `syncEngine.ts` to `quickCapture.ts`, `controlsBlock.ts`?**
