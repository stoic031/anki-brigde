...
## Corpus Check
- 86 files · ~50,747 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 567 nodes · 813 edges · 83 communities (23 shown, 60 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 15 edges (avg confidence: 0.86)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `5e6bf87f`
...
8. `toastError()` - 14 edges
9. `syncNote()` - 13 edges
10. `resolveAnkiConnectUrl()` - 11 edges
...
Cohesion: 0.06
Nodes (40): ALWAYS_VISIBLE_BUTTONS, ControlAction, ControlButtonSpec, DELETE_BUTTON, handleDeleteClick(), handleSync(), performDelete(), registerControlsBlock() (+32 more)
...
Cohesion: 0.05
Nodes (15): AnkiBridgeSettingTab, handleConnect(), renderConnectionSection(), { deckNames, modelNames }, FakeButtonComponent, fakeDiv(), FakeDropdownComponent, FakeEl (+7 more)
...
Cohesion: 0.08
Nodes (43): plugin, $schema, .opencode/plugins/graphify.js, AnkiBridgePlugin, generateContentSkeleton(), runCreateNote(), fakePlugin(), fakeSettings() (+35 more)
...
- **Why does `AnkiConnectClient` connect `controlsBlock.ts` to `quickCapture.ts`, `settingsTab.test.ts`?**
  _High betweenness centrality (0.023) - this node is a cross-community bridge._
- **Why does `NoteNameModal` connect `noteNameModal.test.ts` to `quickCapture.ts`?**
  _High betweenness centrality (0.021) - this node is a cross-community bridge._
- **Why does `AnkiBridgePlugin` connect `quickCapture.ts` to `SidebarView`, `main.test.ts`, `settingsTab.test.ts`?**
